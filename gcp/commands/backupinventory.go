...
func (m *BackupInventoryModule) identifyUnprotectedResources() {
	...
	for diskURL, info := range m.allDisks {
		if !m.disksWithBackups[diskURL] {
			...
			if lootFile := m.LootMap[info.ProjectID]["backup-inventory-commands"]; lootFile != nil {
				lootFile.Contents += fmt.Sprintf(
					"# Unprotected disk: %s (%s) - %dGB\n"+
						"gcloud compute resource-policies create snapshot-schedule %s-backup \\\n"+
						"  --project=%s \\\n"+
						"  --region=%s \\\n"+
						"  --max-retention-days=30 \\\n"+
						"  --daily-schedule\n\n",
					info.Name, info.ProjectID, info.SizeGB,
					info.Name, info.ProjectID, m.extractRegionFromZone(info.Zone),
				)
			}
		}
	}
