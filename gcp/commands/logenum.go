...
var GCPLogEnumCommand = &cobra.Command{
	Use:     globals.GCP_LOGENUM_MODULE_NAME,
	Aliases: []string{"log-scan", "sensitive-logs"},
...
Flags:
  ...
  --log-name     Optional: filter to a specific log name`,
	Run: runGCPLogEnumCommand,
}

func init() {
	GCPLogEnumCommand.Flags().IntVar(&logEnumHours, "hours", 168, "Hours of logs to scan (default 168 = 7 days)")
	GCPLogEnumCommand.Flags().IntVar(&logEnumMaxEntries, "max-entries", 50000, "Maximum log entries to process per project")
	GCPLogEnumCommand.Flags().StringVar(&logEnumLogName, "log-name", "", "Optional: filter to a specific log name")
}
...
func runGCPLogEnumCommand(cmd *cobra.Command, args []string) {
	cmdCtx, err := gcpinternal.InitializeCommandContext(cmd, globals.GCP_LOGENUM_MODULE_NAME)
	if err != nil {
		return
	}
...
func (m *LogEnumModule) Execute(ctx context.Context, logger internal.Logger) {
	logger.InfoM(fmt.Sprintf("Scanning log entries (last %d hours, max %d entries per project)...",
		m.Hours, m.MaxEntries), globals.GCP_LOGENUM_MODULE_NAME)

	m.RunProjectEnumeration(ctx, logger, m.ProjectIDs, globals.GCP_LOGENUM_MODULE_NAME, m.processProject)

	allEntries := m.getAllEntries()
	if len(allEntries) == 0 {
		logger.InfoM("No sensitive log entries found", globals.GCP_LOGENUM_MODULE_NAME)
		return
	}
	...
	logger.SuccessM(fmt.Sprintf("Found %d sensitive log entries (%d CRITICAL, %d HIGH)",
		len(allEntries), criticalCount, highCount), globals.GCP_LOGENUM_MODULE_NAME)
...
func (m *LogEnumModule) processProject(ctx context.Context, projectID string, logger internal.Logger) {
	if globals.GCP_VERBOSITY >= globals.GCP_VERBOSE_ERRORS {
		logger.InfoM(fmt.Sprintf("Scanning logs in project: %s", projectID), globals.GCP_LOGENUM_MODULE_NAME)
	}
	...
	if err != nil {
		...
		gcpinternal.HandleGCPError(err, logger, globals.GCP_LOGENUM_MODULE_NAME,
	...
	if len(entries) > 0 {
		...
		m.LootMap[projectID]["log-enum-commands"] = lootFile
	}
	m.mu.Unlock()
}
...
func (m *LogEnumModule) writeHierarchicalOutput(ctx context.Context, logger internal.Logger) {
	...
	if err != nil {
		logger.ErrorM(fmt.Sprintf("Error writing hierarchical output: %v", err), globals.GCP_LOGENUM_MODULE_NAME)
	}
}
...
func (m *LogEnumModule) writeFlatOutput(ctx context.Context, logger internal.Logger) {
	...
	if err != nil {
		logger.ErrorM(fmt.Sprintf("Error writing output: %v", err), globals.GCP_LOGENUM_MODULE_NAME)
	}
}
