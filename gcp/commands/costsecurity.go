...
func (m *CostSecurityModule) checkCryptominingIndicators(instance *compute.Instance, projectID, zone, machineType string, vcpus int64, memGB float64, gpuCount int) {
	...
	for _, ind := range indicators {
		...
		// Add to loot
		m.LootMap["cost-anomalies"].Contents += fmt.Sprintf(
			"## CRYPTOMINING INDICATOR: %s\n"+
				...
				"Details: %s\n"+
				"Created: %s\n\n",
			ind.Name, ind.ProjectID, ind.Location,
			ind.Indicator, ind.Confidence, ind.Details, ind.CreatedTime,
		)
	}
	m.mu.Unlock()
}
		...
		for zone, diskList := range page.Items {
			...
			for _, disk := range diskList.Disks {
				...
				if len(disk.Users) == 0 {
					...
					m.mu.Lock()
					m.LootMap["orphaned-resources"].Contents += fmt.Sprintf(
						"%s (disk, %dGB) - %s\n# Delete: gcloud compute disks delete %s --zone=%s --project=%s\n\n",
						disk.Name, disk.SizeGb, orphaned.Reason,
			...
			for _, addr := range addressList.Addresses {
				...
				if addr.Status == "RESERVED" && len(addr.Users) == 0 {
					...
					m.LootMap["orphaned-resources"].Contents += fmt.Sprintf(
						...
						addr.Name, addr.Address, orphaned.Reason,
...
func (m *CostSecurityModule) initializeLootFiles() {
	m.LootMap["cost-anomalies"] = &internal.LootFile{
		Name:     "cost-anomalies",
		Contents: "# Cost Anomalies and Potential Cryptomining\n# Generated by CloudFox\n# CRITICAL: Review these findings immediately!\n\n",
	}
	m.LootMap["orphaned-resources"] = &internal.LootFile{
		Name:     "orphaned-resources",
		Contents: "# Orphaned Resources (Cleanup Commands)\n# Generated by CloudFox\n\n",
	}
	m.LootMap["cleanup-commands"] = &internal.LootFile{
		Name:     "cleanup-commands",
		Contents: "# Resource Cleanup Commands\n# Generated by CloudFox\n# Review before executing!\n\n",
	}
}
...
func (m *CostSecurityModule) writeOutput(ctx context.Context, logger internal.Logger) {
	// Sort cryptomining indicators by confidence
	sort.Slice(m.Cryptomining, func(i, j int) bool {
		confOrder := map[string]int{"HIGH": 0, "MEDIUM": 1, "LOW": 2}
		return confOrder[m.Cryptomining[i].Confidence] < confOrder[m.Cryptomining[j].Confidence]
	})
	...
	for _, o := range m.Orphaned {
		orphanedBody = append(orphanedBody, []string{
			...
			o.ResourceType,
			o.Location,
			fmt.Sprintf("%d", o.SizeGB),
			fmt.Sprintf("$%.2f", o.EstCostMonth),
			truncateString(o.Reason, 30),
		})
	}
	...
	for _, a := range m.CostAnomalies {
		...
		// Add to cleanup commands loot
		if a.Remediation != "" {
			m.LootMap["cleanup-commands"].Contents += fmt.Sprintf(
				"# %s (%s) - %s\n%s\n\n",
				a.Name, a.AnomalyType, a.Details, a.Remediation,
			)
		}
	}
	...
	if len(cryptoBody) > 0 {
		tables = append(tables, internal.TableFile{
			Name:   "cryptomining-indicators",
			Header: cryptoHeader,
			Body:   cryptoBody,
		})
	}

	if len(orphanedBody) > 0 {
		tables = append(tables, internal.TableFile{
			Name:   "orphaned-resources",
			Header: orphanedHeader,
			Body:   orphanedBody,
		})
	}

	if len(anomaliesBody) > 0 {
		tables = append(tables, internal.TableFile{
			Name:   "cost-anomalies",
