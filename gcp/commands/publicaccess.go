...
func (m *PublicAccessModule) Execute(ctx context.Context, logger internal.Logger) {
	...
	allUsersCount := 0
	allAuthCount := 0
	for _, r := range m.PublicResources {
	...
	logger.SuccessM(fmt.Sprintf("Found %d public resource(s): %d allUsers, %d allAuthenticatedUsers",
		len(m.PublicResources), allUsersCount, allAuthCount), globals.GCP_PUBLICACCESS_MODULE_NAME)
...
func (m *PublicAccessModule) addResource(resource PublicResource) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.PublicResources = append(m.PublicResources, resource)
...
// ------------------------------
func (m *PublicAccessModule) initializeLootFiles() {
	m.LootMap["public-access-commands"] = &internal.LootFile{
...
func (m *PublicAccessModule) addResourceToLoot(resource PublicResource) {
	m.LootMap["public-access-commands"].Contents += fmt.Sprintf(
		"## [%s] %s: %s (Project: %s)\n"+
			"# Access: %s\n"+
			"# Role: %s\n",
	...
	case "Source Repository":
		m.LootMap["public-access-commands"].Contents += fmt.Sprintf(
			...
			resource.ResourceName, resource.ProjectID)
	default:
		m.LootMap["public-access-commands"].Contents += "\n"
	}
}
...
func (m *PublicAccessModule) writeOutput(ctx context.Context, logger internal.Logger) {
	...
	for _, loot := range m.LootMap {
		if loot.Contents != "" && !strings.HasSuffix(loot.Contents, "# WARNING: These resources are publicly accessible!\n\n") {
