...
func (m *CertManagerModule) processProject(ctx context.Context, projectID string, logger internal.Logger) {
	...
	if m.LootMap[projectID] == nil {
		...
		m.LootMap[projectID]["certmanager-details"] = &internal.LootFile{
	...
	for _, cert := range certs {
		m.addCertToLoot(projectID, cert)
	}
	for _, cert := range sslCerts {
		m.addSSLCertToLoot(projectID, cert)
	}
	m.mu.Unlock()
}
...
// ------------------------------
func (m *CertManagerModule) addCertToLoot(projectID string, cert certmanagerservice.Certificate) {
	lootFile := m.LootMap[projectID]["certmanager-details"]
	if lootFile == nil {
		return
	}
...
func (m *CertManagerModule) addSSLCertToLoot(projectID string, cert certmanagerservice.SSLCertificate) {
	lootFile := m.LootMap[projectID]["certmanager-details"]
	if lootFile == nil {
		return
	}
...
func (m *CertManagerModule) writeHierarchicalOutput(ctx context.Context, logger internal.Logger) {
	...
	for projectID := range projectIDs {
		...
		outputData.ProjectLevelData[projectID] = CertManagerOutput{Table: tableFiles, Loot: lootFiles}
	}
...
func (m *CertManagerModule) writeFlatOutput(ctx context.Context, logger internal.Logger) {
	...
	output := CertManagerOutput{Table: tables, Loot: lootFiles}
