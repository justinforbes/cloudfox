...
func (m *BigQueryModule) initializeLootFiles() {
	...
	m.LootMap["bigquery-views"] = &internal.LootFile{
		Name:     "bigquery-views",
		Contents: "# GCP BigQuery Views\n# Generated by CloudFox\n# Views may expose data from other datasets\n\n",
	}
	m.LootMap["bigquery-google-managed-encryption"] = &internal.LootFile{
		Name:     "bigquery-google-managed-encryption",
		Contents: "# Datasets Using Google-Managed Encryption\n# Generated by CloudFox\n# Consider CMEK for compliance requirements\n\n",
	}
	m.LootMap["bigquery-cmek"] = &internal.LootFile{
		Name:     "bigquery-cmek",
		Contents: "# Datasets Using CMEK (Customer-Managed Encryption Keys)\n# Generated by CloudFox\n\n",
	}
	m.LootMap["bigquery-security-recommendations"] = &internal.LootFile{
		Name:     "bigquery-security-recommendations",
		Contents: "# BigQuery Security Recommendations\n# Generated by CloudFox\n# Remediation commands for security issues\n\n",
	}
	m.LootMap["bigquery-large-tables"] = &internal.LootFile{
		Name:     "bigquery-large-tables",
		Contents: "# Large BigQuery Tables (>1GB)\n# Generated by CloudFox\n# These tables may contain significant data\n\n",
	}
}
...
func (m *BigQueryModule) addDatasetToLoot(dataset BigQueryService.BigqueryDataset) {
	...
	// Encryption status
	if dataset.EncryptionType == "Google-managed" || dataset.EncryptionType == "" {
		m.LootMap["bigquery-google-managed-encryption"].Contents += fmt.Sprintf(
			"# DATASET: %s (Project: %s, Location: %s)\n"+
	...
	} else if dataset.EncryptionType == "CMEK" {
		m.LootMap["bigquery-cmek"].Contents += fmt.Sprintf(
			"# DATASET: %s (Project: %s, Location: %s)\n"+
				"# Encryption: CMEK\n"+
				"# KMS Key: %s\n\n",
			dataset.DatasetID, dataset.ProjectID, dataset.Location, dataset.KMSKeyName,
		)
	}

	// Security recommendations
	m.addDatasetSecurityRecommendations(dataset)
}

// addDatasetSecurityRecommendations adds remediation commands for dataset security issues
func (m *BigQueryModule) addDatasetSecurityRecommendations(dataset BigQueryService.BigqueryDataset) {
	...
	// Google-managed encryption (consider CMEK)
	if dataset.EncryptionType == "Google-managed" || dataset.EncryptionType == "" {
		hasRecommendations = true
		recommendations += fmt.Sprintf(
			"# Issue: Using Google-managed encryption\n"+
				"# Enable CMEK with:\n"+
				"bq update --destination_kms_key=projects/PROJECT/locations/LOCATION/keyRings/RING/cryptoKeys/KEY %s:%s\n\n",
			dataset.ProjectID, dataset.DatasetID,
		)
	...
	if hasRecommendations {
		m.LootMap["bigquery-security-recommendations"].Contents += recommendations + "\n"
	}
}
...
func (m *BigQueryModule) addTableToLoot(table BigQueryService.BigqueryTable) {
	...
	if table.IsView {
		...
		m.LootMap["bigquery-views"].Contents += fmt.Sprintf(
			"# VIEW: %s.%s (Project: %s)\n"+
				"# Type: %s\n"+
				"# Legacy SQL: %v\n"+
				"# Query:\n"+
	...
	if table.NumBytes > oneGB {
		sizeGB := float64(table.NumBytes) / float64(oneGB)
		m.LootMap["bigquery-large-tables"].Contents += fmt.Sprintf(
			"# TABLE: %s.%s (Project: %s)\n"+
				"# Size: %.2f GB (%d bytes)\n"+
				"# Rows: %d\n"+
				"# Type: %s\n"+
